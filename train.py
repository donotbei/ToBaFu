...
from model.topo_model_new3 import get_Topo_model as TopoModel
from model.img_model_new3 import get_Img_model as ImgModel
#conv2d,ReLU,flatten,Dropout,FC
...
def train_img_model(
    ...
    # define the early stopping
    ckpt_path = os.path.join(ckpt_dir, 'img_checkpoint.pt')
    early_stopping = EarlyStopping(patience=patience, verbose=True, path=ckpt_path)
    ...
    # visualize the losses
    visualize_losses(*losses, os.path.join(ckpt_dir, 'img_losses.png'))

    return

def train():
    config_data = config['data']
    topo_config = config['train']['topo']
    img_config = config['train']['img']
    ckpt_dir = config['train']['ckpt_dir']
    current_time = datetime.now().strftime("%m%d-%H%M%S")
    ckpt_dir = os.path.join(ckpt_dir, 'ToBaFu' + '-' + current_time)
    os.makedirs(ckpt_dir, exist_ok=True)
    print("Checkpoint directory:", ckpt_dir)

    # get the input size and num_classes
    topo_train_loader, img_train_loader = read_data(config_data, 'train')
    combine_loader = read_data(config_data, 'test')
    ...
    train_img_model(num_classes, img_config, img_train_loader, combine_loader, ckpt_dir)
